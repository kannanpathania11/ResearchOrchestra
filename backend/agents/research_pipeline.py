...
# ─────────────────────────────────────────────────────────────────────────────
# MODE CONFIGURATION
# Centralised lookup so every node can stay clean.
# ─────────────────────────────────────────────────────────────────────────────
...
def _meta(state: ResearchState) -> Dict:
    """Return the mode metadata for the current research_mode."""
...
async def node_clarify(state: ResearchState) -> ResearchState:
    ...
    prompt = (
        ...
        f"User Input: \"\"\"{state['user_prompt']}\"\"\"\n\n"
        f"Your goal is to {m['clarify_goal']}\n\n"
        f"Return JSON with keys:\n"
        f"- needs: boolean\n"
        f"- questions: list of short questions (max {m['clarify_limit']})\n"
        f"If nothing is needed, needs=false and questions=[]."
    )
    ...
    if needs and questions:
        await adispatch_custom_event(
            "research_update", {"message": "Clarification needed", "questions": questions}
        )
    else:
        await adispatch_custom_event(
...
# ─────────────────────────────────────────────────────────────────────────────
# NODE 3 – REFLECT  (new node — validates search coverage before synthesis)
# ─────────────────────────────────────────────────────────────────────────────

MAX_REFLECT_PASSES = 1   # Allow at most one extra search round

async def node_reflect(state: ResearchState) -> ResearchState:
    """
    Self-critique node. Checks whether the fetched pages provide enough coverage
    to write a high-quality report. If not (and we haven't already retried), it
    generates supplementary queries and loops back to search/fetch.
    """
    ...
    # Fast-path: already retried once, proceed regardless
    if reflect_pass >= MAX_REFLECT_PASSES or len(pages) >= 6:
        await adispatch_custom_event(
            "research_update", {"message": "Coverage sufficient. Proceeding to synthesis."}
        )
        return {**state, "reflect_pass": reflect_pass}  # no change → synthesize

    # Ask the LLM if coverage is good enough
    snippet = "\n".join(
    ...
    prompt = (
        ...
        f"Is this coverage sufficient to write a comprehensive, data-rich report?\n"
        f"If YES, reply with exactly: SUFFICIENT\n"
        f"If NO, reply with exactly: INSUFFICIENT, then list 3 additional specific search "
        f"queries (one per line, no numbering) to fill the gap."
    )
    ...
    # Parse supplementary queries
    lines = [l.strip("- ").strip() for l in text.splitlines() if l.strip() and "INSUFFICIENT" not in l.upper()]
    extra_queries = [l for l in lines if l][:3]

    if extra_queries:
        await adispatch_custom_event(
            "research_update",
            {"message": f"Coverage gap detected. Running {len(extra_queries)} supplementary searches...",
             "queries": extra_queries},
        )
        # Merge with existing queries and signal a re-search
        return {
            **state,
            "subqueries": extra_queries,
            "hits": [],           # clear hits so node_search starts fresh
...
def _reflect_router(state: ResearchState) -> str:
    """Route after reflect: re-search if flagged, otherwise synthesize."""
    if state.get("_needs_extra_search"):
        return "search"
    return "synthesize"
...
builder.add_node("query_planner", node_query_planner)
...
# clarify → query_planner  OR  END (INTERRUPT for clarification)
builder.add_conditional_edges(
    "clarify",
    lambda s: "query_planner" if not s["needs_clarification"] else "INTERRUPT",
    {"query_planner": "query_planner", "INTERRUPT": END},
)

# query_planner → search → fetch → reflect
builder.add_edge("query_planner", "search")
builder.add_edge("search",        "fetch")
builder.add_edge("fetch",         "reflect")

# reflect → synthesize  OR  loop back to search (supplementary queries)
builder.add_conditional_edges(
    "reflect",
    _reflect_router,
